...
LoRaWAN = lorawan_ns.class_("LoRaWAN", cg.PollingComponent)
...
def validate_lorawan_sensor(config):
...
def lorawan_sensor_schema(base_schema):
    return cv.All(
        base_schema.extend(
            {
                cv.GenerateID(CONF_TRANSPORT_ID): cv.use_id(LoRaWAN),
...
async def register_lorawan(var, config):
...
async def new_lorawan(config):
    var = cg.new_Pvariable(config[CONF_ID])
    cg.add(var.set_platform_name(config[CONF_PLATFORM]))
    providers = await register_lorawan(var, config)
